...
export END_PATH="/path/to/mattergenbis/results/Vladimir"  # Directory containing the zip files
...
systems=(Cu-Si-K Cu-Si-P Mo-Si-K Mo-Si-P Mo-Si-B Mo-Si-B-P Si-B-P Mo-B-P
         ...
         #Mo Cu Si K P B)
mkdir -p /path/to/mattergenbis/$RESULTS_PATH
mkdir -p $END_PATH
...
for system in "${systems[@]}"; do
    ...
    mkdir -p /path/to/mattergenbis/$RESULTS_PATH$system
    mattergen-generate "$RESULTS_PATH$system" --pretrained-name=$MODEL_NAME --batch_size=$NB --properties_to_condition_on="{'chemical_system':'$system'}" --record_trajectories=False --diffusion_guidance_factor=2.0 > /path/to/mattergenbis/$RESULTS_PATH$system/log.txt 2>&1
    # Copy the generated samples to the end path
    cp -r "/path/to/mattergenbis/$RESULTS_PATH$system/generated_crystals_cif.zip" "$END_PATH/$system.zip"
    end_time=$(date +%s)
...
cd /path/to/mattergenbis/results/
